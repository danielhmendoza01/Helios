...
int main() {
    ...
    /******/
    gzFile logFile = createGzFile("/scratch/dmendoza/Helios_Project/fastBoyWin/files/logs/log.txt.gz");
    ...
    while (true) {
        ...
        while ((pos = buffer.find('\n')) != string::npos) {
            ...
            if (lineIndex == 3) {
                //boyer
                search(lines[1], pattern, lines[3], logFile);
                /******/
                if (lines[1].size() < 50 || lines[3].size() < 50)
                {
                    gzWriteStringToGzFile(logFile, "********BOYER BASE ERROR*******\n");
                }
                if(lines[1].size() != lines[3].size()){
                    gzWriteStringToGzFile(logFile, "********BOYER SAME LENGTH ERROR*******\n");
                }
                /*******/
                ...
                if (lines[1].size() < 50 || lines[3].size() < 50)
                {
                    gzWriteStringToGzFile(logFile, "********WINDOW BASE ERROR*******\n");
                }
                if(lines[1].size() != lines[3].size()){
                    gzWriteStringToGzFile(logFile, "********WINDOW SAME LENGTH ERROR*******\n");
                }
                /*******/
